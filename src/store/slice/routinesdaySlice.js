  ...
  reducers: {
        ...
        return {
          ...
          routine_items: items.routine_items.map((item) =>
            item.id == IdEx
              ? {
                  ...item,
                  routine_sets: item.routine_sets.map((sets, index) =>
                    index == IdSet
                      ? { ...sets, done: check, numberDone: 1 }
                      : sets
                  ),
                }
              : item
    ...
    updateDeleteExercise: (state, { payload }) => {
      state.list = state.list.map((items) => {
        return {
          ...items,
          routine_items: items.routine_items.filter(
    ...
    setUpdateInputNumber: (state, { payload }) => {
      ...
      const newOject = state.list.map((it) =>
        it.routine_items.map((ite) =>
          ite.routine_sets.map((se) => se.amount.map((am) => am[0]))
        )
      );
      state.list = state.list.map((items) => {
        return {
              ...
              ? {
                  ...
                  routine_sets: item.routine_sets.map((sets, index) =>
                    sets.id == SetId
                      ? {
                          ...sets,
                          amount: sets.amount.map(
                            (amounts) =>
                              amounts[0].index_id == Index_Id
                                ? [{ index_id: Index_Id, amount: number }]
                                : [
                                    {
                                      index_id: amounts[0].index_id,
        ...
        return {
          ...
          routine_items: items.routine_items.map((item) =>
            item.id == id
              ? {
                  ...item,
                  routine_sets: item.routine_sets.filter(
                    (sets, index) => index !== indexSet
                  ),
                }
              : item
          ),
    ...
    updateAddSuperSet: (state, { payload }) => {
      ...
      state.list.map((item) =>
        item.routine_items.map((items) =>
          items.id == SuperSetKey
            ? state.super_set.push([
                { key: SuperSetKey, id: id },
                { key: SuperSetKey, id: items.exercise_id },
              ])
            : ""
        )
      );
...
export const {
  ...
  deleteSet,
  updateAddExercise,
  setUpdateInputNumber,
} = routinesdaySlice.actions;
