...
import CheckBox from "../../routines/checkBox";
...
import ListExercisesRoutin from "../../routines/listExercisesRoutin";
...
const CardRoutineDay = () => {
  ...
  useEffect(() => {
    if (successAPI == true) {
      routinApi
        .get(`/routine/history/show/${param.id}`)
        .then((result) => {})
        .catch((err) => "");
    }
  }, []);
  ...
  const handleEndRoutin = () => {
    list[0]?.routine_items.map((item) =>
      item.routine_sets.map((sets, index) =>
        setnumberSet((previousState) => {
    ...
    list[0]?.routine_items.map((item) =>
      item.routine_sets.map((sets, index) =>
        setNumberdone((prev) => (sets.done == true ? prev + 1 : prev))
      )
    );
  };
  ...
  const handleReplace = (replace) => {
    if (replace) return handleOpenModal();
  };
  ...
  const handleSuperSet = (superSet) => {
    if (superSet) return handleOpenSuperSet();
  };
      ...
      <div className="lg:container md:direction: rtl mx-auto ">
        {error ? (
          <>
            <CustomizedSnackbars
              variant={3}
              text={"لطفا تمام ست ها را انجام دهید"}
            />
            {setTimeout(() => {
              setError(false);
            }, 3000)}
            ...
            <>
              <>
                <div className="hidden md:flex">
                  <div className={`${start ? "hidden" : "flex"}`}>
                    <Button
                      onClick={handleStartRoutin}
                      variant="contained"
                      className=" h-10 button float-left "
                      color="primary"
                    >
                      <Typography> شروع روتین </Typography>
                    </Button>
                  </div>
                  <div className={`${start ? "flex" : "hidden"} `}>
                    <Button
                      onClick={handleEndRoutin}
                      variant="contained"
                      className=" h-10 button float-right "
                      color="warning"
                    >
                      <Typography> اتمام روتین </Typography>
                    </Button>
                  </div>
                  <div className="mr-20">
                    <TimerRoutin start={start} />
                  </div>
                </div>
                <div className=" md:hidden">
                  ...
                  >
                    ...
                    >
                      <Typography className=" "> شروع روتین </Typography>
                    </Button>
                  </div>
                  <div
                    className={`mb-3 justify-center ${
                    ...
                    <Card key={routes.id} sx={{ maxWidth: 700, marginTop: 5 }}>
                      <CardHeader
                        ...
                        action={
                          <LongMenu
                            Id={routes?.id}
                            exercise_id={routes.exercise_id}
                            getSuperSet={(superSet) => handleSuperSet(superSet)}
                            getSuperSetRoutin={(id) => addSuperSet(id)}
                            getReplace={(replace) => handleReplace(replace)}
                            seperator={2}
                          />
                        }
                        title={
                          <>
                            <h1 className="title-card ">
                              {routes?.exercise.fa_title}
                            </h1>
                            <>
                              <div className="hidden md:flex">
                                {super_set?.map((item, index) =>
                                  item?.map((setId, ind) =>
                                    setId?.id == routes?.exercise_id ? (
                                      <div key={index + 25}>
                                        <div
                                          onDoubleClick={() =>
                                            handleDeleteSuperSetHistory({
                                              id: item[0].id,
                                              IndexSuper: index,
                                            })
                                          }
                        ...
                        >
                          <TableHead key={101}>
                            <TableRow key={102} className="">
                              <TableCell key={103} className="" align="right">
                                <CheckIcon className="mr-2" />
                              </TableCell>
                              <TableCell key={104} className="" align="right">
                                <Typography>ست</Typography>
                              </TableCell>
                              {routes.exercise?.type.indices?.map(
                                (type, ind) => (
                                  <TableCell
                                    className=""
                                    key={ind + 600}
                            ...
                            {routes?.routine_sets?.map((sets, indexSet) => (
                              <>
                                <TableRow
                                  key={indexSet + 200}
                                  className={`${
                                    sets?.done == true ? "row-done" : ""
                                  } `}
                                >
                                  <TableCell
                                    key={indexSet + 300}
                                    className="bg-emerald-50  w-4 md:w-10"
                                    align="right"
                                  >
                                    {start == true ? (
                                      <CheckBox
                                  ...
                                  {sets?.amount?.map((item, index) => (
                                    ...
                                    >
                                      {
                                        <InputAddNumberRoutinDay
                                          key={index}
                                          IndexSet={indexSet}
                                          Id={routes.id}
                                          separator={2}
                                          Index_Id={item[0]?.index_id}
                                          SetId={sets.id}
                                          amountDay={item}
                                          unit={sets.unit}
                                        />
                      ...
                      <Grid sx={{ m: 3 }}>
                        <Button
                          sx={{ width: "100%" }}
                          onClick={() => handleAddSet(routes.id)}
                          size={"large"}
                          variant="contained"
                          className="float-end mt-5  h-10  button"
                        >
                          <Typography className="">اضافه کردن ست</Typography>
                        </Button>
                      </Grid>
                      <Modal
                        open={openSuperSet}
